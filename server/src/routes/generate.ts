...
import { generateImage, checkComfyStatus, ComfyError, GENERATED_DIR, sanitizeSegment } from '../services/comfyui.js';
...
generateRouter.post('/image', async (req, res) => {
  ...
  try {
    ...
    console.error(`Generation failed for ${key}:`, err instanceof Error ? err.message : String(err));
    if (err instanceof ComfyError) {
      ...
      res.status(500).json({ error: errorMessage || 'Unknown generation error', code: 'UNKNOWN' });
    }
  } finally {
